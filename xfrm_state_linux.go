...
func XfrmStateAdd(s *NetlinkSocket, state *XfrmState) error {
	...
	if state.Aead != nil {
		out := NewRtAttr(XFRMA_ALG_AEAD, writeStateAlgo(state.Aead))
		req.AddData(out)
	}
	if state.Encap != nil {
...
func XfrmStateDel(s *NetlinkSocket, state *XfrmState) error {
...
func XfrmStateList(s *NetlinkSocket, family int) ([]XfrmState, error) {
	req := NewNetlinkRequest(XFRM_MSG_GETSA, syscall.NLM_F_DUMP)

	msg := NewIfInfomsg(family)
	req.AddData(msg)

	msgs, err := req.Execute(s, XFRM_MSG_NEWSA)
	if err != nil {
		return nil, err
	}
